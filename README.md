...
**[v0.2.2-t5](https://github.com/ovo4096/cubie-a7z-archlinuxarm/releases/tag/v0.2.2-t5)** 为三个变体加入共用的 HDMI 内核修复：避免 HPD 短暂断连后硬件输出与 DRM 状态不一致，移除 KDE / XFCE 的旧启动兜底，保留真实连接检测，不固定显示器或分辨率。此前 CLI 在无兜底、`hpd_mask=0` 下完成两次启动和一次真实拔插，画面均正常。**本次新包、六张镜像及桌面未做新的自动化、滚动升级或实机测试。** 限制与迁移方法见 [HDMI 说明](desktop/HDMI.zh-CN.md)。
...
| 变体 | 默认环境 | 主要配置 |
|---|---|---|
| `cli` | SSH、NetworkManager、维护工具，无桌面 | 共用 HDMI 内核修复；适合无显示器运行或作为 UFS 安装用 SD；不安装浏览器和输入法 |
| `xfce` | XFCE + LightDM + Arch Xorg | 共用 HDMI 内核修复；XRender / XPresent 合成、Chromium（PowerVR）、Fcitx5 + Rime |
| `kde` | Plasma X11 + SDDM | 共用 HDMI 内核修复；Qt Quick 使用 PowerVR Vulkan，同一 Chromium / Rime；KWin 合成关闭 |
...
```sh
sudo python3 tools/build.py --variant xfce --work-dir /root/a7z-build/xfce --hdmi-kernel-input /path/to/hdmi-kernel-input
sudo python3 tools/build.py --variant cli --work-dir /root/a7z-build/cli --hdmi-kernel-input /path/to/hdmi-kernel-input
sudo python3 tools/build.py --variant kde --work-dir /root/a7z-build/kde --hdmi-kernel-input /path/to/hdmi-kernel-input
```

`--hdmi-kernel-input` 必须指向本补丁的内核输入目录；可从发布页取得配套输入包并解压，或按 [内核构建说明](kernel/hdmi/README.md) 从锁定源码生成。不能用官方原版 Image 冒充补丁内核。每个变体必须使用独立、全新的工作目录。Windows 可从 PowerShell 运行同样的 Python 命令，工具会转交给 WSL。默认 XFCE；`--plan` 只预览，`--resume` 只复用源锁、配方、变体和配置均相同的构建目录。失败后的目录保留，不递归删除用户目录；需要重新解包时选择新的工作目录。
...
v0.2.2 是内核补丁发行版：复用构建机上同变体的干净离线 rootfs 和既有 T5 包快照，复制到新构建卷后安装 `linux-radxa-a7z 6.6.98_4-2`、`radxa-a7z-base 0.1.0-6` 及依赖匹配的板级包，清理旧 HDMI 桌面钩子。随后执行公开镜像清理，生成带独立 UUID 的六张镜像。本轮未执行 `pacman -Syu`，没有新增 Chromium、Rime 或 GPU / VPU 功能改动。这个补丁发行流程与上面的通用全流程构建命令不同，实际输入及产物哈希以发布页构建记录为准；没有从测试板导入系统、网络配置或用户数据。
...
仓库保留开发测试命令，供后续开发运行；**v0.2.2 发布没有执行这些测试，也没有开展新镜像硬件启动或滚动升级回归**。此前 CLI 内核补丁确认及 v0.2.0 测试记录属于限定范围的历史结果，不作为本版六张新镜像的验收结论。
